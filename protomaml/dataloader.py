...
class DataTwitterDavid(Dataset):
    """ Test dataset class.
    All other dataset classes should include task_name and n_classes."""
    def __init__(self, csv_file_dir: str):
        ...
        with open(csv_file_dir, mode='r') as csvfile:
            ...
            for row in reader:
                ...
                self.label.append(torch.Tensor(int(row['class']),
                                               dtype=torch.long))
    ...
    def __getitem__(self, idx):
        return self.tweets[idx], self.class[idx]  # TODO "class" does not exist, also overwriting the class keyword is not very pretty
...
def prepare_batch(batch, tokenizer="bert-base-uncased"):
    ...
    texts = tokenizer(texts, padding=True, truncation=True,
                      return_tensors="pt")
...
def generate_tasks_from_dataset(dataset, num_tasks=None, support_examples=100,
    ...
    for i in range(interval):
        ...
        tasks.append(Task(
            ...
            support_loader=create_dataloader(
                support,
